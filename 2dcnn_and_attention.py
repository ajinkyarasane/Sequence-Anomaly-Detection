...
Original file is located at
    https://colab.research.google.com/drive/1iX7vi7Gze7-MhCvMlgT1MqPBBE_hxNbO
"""
...
class get_data():
  def __init__(self,train_ids,y_dat,y_hot):
    self.Ambi_Npy=sorted(os.listdir("/content/gdrive/MyDrive/New-FSD50K-Dataset/Ambi_pann"))
    self.Animal_Npy=sorted(os.listdir("/content/gdrive/MyDrive/New-FSD50K-Dataset/Animal_pann"))
    self.Human_Npy=sorted(os.listdir("/content/gdrive/MyDrive/New-FSD50K-Dataset/Human_pann"))
    self.Music_Npy=sorted(os.listdir("/content/gdrive/MyDrive/New-FSD50K-Dataset/Music_pann"))
    self.Nature_Npy=sorted(os.listdir("/content/gdrive/MyDrive/New-FSD50K-Dataset/Nature_pann"))
    self.SoT_Npy=sorted(os.listdir("/content/gdrive/MyDrive/New-FSD50K-Dataset/SoT_pann"))
    self.x_vec=train_ids
    X_path="/content/gdrive/MyDrive/New-FSD50K-Dataset/"
...
test_data=get_data(X_test["Fname"].tolist(),y_train["y_category"].to_list(),y_train["y_hot"].to_list())
...
class SelfAttentionPooling(nn.Module):
    ...
    def _init_(self, temperature, attn_dropout=0.1):
        super()._init_()
        self.temperature = 527
        self.dropout = nn.Dropout(attn_dropout)

    def forward(self, q, k, v, mask=None):
        B, key_seq_max_len, key_value_size = k.shape
        range_L = torch.arange(key_seq_max_len).unsqueeze(1)
        mask = torch.transpose(range_L >= key_seq_max_len, 1, 0)
        mask = mask.to(device)
        attn = torch.matmul(q / 527, k.transpose(1, 2))
        if mask is not None:
...
class SelfAtt(nn.Module):
  def __init__(self):
    ...
    self.l1 = SelfAttentionPooling()
    self.l2 = nn.Linear(527,6)
  def forward(self,x):
    op1 = self.l1( q = x, k = x, v= x)
    op2 = self.l2(op1)
    return op2
...
class Network(torch.nn.Module):
    ...
    def __init__(self):
        ...
        in_size = 9216
        layers = [
                  nn. Conv2d(1,3,kernel_size=3,stride=3),
                  nn.BatchNorm2d(3),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=3),
            nn. Conv2d(3,3,kernel_size=3,stride=3),
            nn.BatchNorm2d(3),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=3),
            nn.Flatten()
        ...
        self.linear=nn.Sequential(nn.Linear(1530,1000),nn.BatchNorm1d(1000),nn.ReLU(),nn.Linear(1000,500),nn.ReLU(),nn.Linear(500,6))
...
def val_score():
  ...
  with torch.no_grad():
    ...
    for i, (data,target) in enumerate(val_loader):
      if (i%100==0):
        print(i)
      data = data.float().reshape(data.shape[0],data.shape[1],data.shape[2]).to(device)
      # target = target.float().to(device)
      output = model(data)
      # output=sig(output)
      model_result.extend(output.cpu().numpy())
...
for ep in range(epochs):
  ...
  for i, (data,target) in enumerate(train_loader):
    ...
    optimizer.step()
  scheduler.step(avg_loss)
  average_loss.append((avg_loss/len(train_dataset)) * 128)
  print("============EPOCH : ",ep,"=======================")
  print("average loss epoch: ",avg_loss)
  print("average loss epoch: ",avg_loss/len(train_dataset))
  val_score()
